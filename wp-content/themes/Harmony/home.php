...
<!-- Poste Recente Cometados- Tirar o PHP que comenta essa parte--><!--
<div id="news-section">
	...
	<?php if ( 'on' == $display_news_section ) { ?>
		...
		<div id="news">
			<h2><?php echo esc_html( et_get_option( 'harmony_recent_news_title', __( 'Posts recentes', 'Harmony' ) ) ); ?></h2>
			<ul id="news-posts">
			...
			?>
				...
				<?php
					printf ( __( '<p class="meta-info">Postado por <a href="%1$s">%2$s</a> em %3$s</p>', 'Harmony' ),
		...
		<?php if ( ( $more_news_link = et_get_option( 'harmony_more_news_link' ) ) && '' != $more_news_link ) { ?>
			<a href="<?php echo esc_url( $more_news_link ); ?>" class="more"><?php esc_html_e( 'Mais notícias', 'Harmony' ); ?></a>
		<?php } ?>
		</div> <!-- end #news -->
<!-- /Post Recentes Comentados -->
	<?php } ?>
